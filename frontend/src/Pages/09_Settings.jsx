...
function App() {
  ...
  const [gymLogoUrl, setGymLogoUrl] = useState('https://placehold.co/150x150/000000/FFFFFF?text=Gym+Logo');
  ...
  const [membershipPlans, setMembershipPlans] = useState([
    { id: 1, name: 'Basic', price: 30, duration: '1 Month', description: 'Access to gym floor' },
    { id: 2, name: 'Premium', price: 50, duration: '1 Month', description: 'Access to gym floor, classes' },
    { id: 3, name: 'Annual VIP', price: 500, duration: '12 Months', description: 'All access, personal trainer sessions' },
  ]);
  ...
  const handleAddPlan = () => {
    if (!newPlan.name || !newPlan.price || !newPlan.duration || !newPlan.description) {
      alert('Please fill in all fields for the new membership plan.');
      return;
    }
    setMembershipPlans([
      ...membershipPlans,
      { id: Date.now(), ...newPlan } // Use Date.now() for a unique ID
    ]);
  ...
  const handleUpdatePlan = () => {
    ...
    setMembershipPlans(membershipPlans.map(plan =>
      plan.id === editingPlanId ? { ...newPlan, id: editingPlanId } : plan
    ));
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4 font-inter">
      <div className="bg-white shadow-lg rounded-xl p-8 w-full max-w-4xl">
        <h1 className="text-4xl font-bold text-gray-800 mb-8 text-center">Gym Settings</h1>

        {/* Tab Navigation */}
        <div className="flex border-b border-gray-200 mb-8">
          <button
            className={`py-3 px-6 text-lg font-medium rounded-t-lg transition-all duration-200
              ${activeTab === 'gymInfo' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
          ...
          <button
            className={`py-3 px-6 text-lg font-medium rounded-t-lg transition-all duration-200
              ${activeTab === 'membershipPlans' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:bg-gray-100'}`}
        ...
        {activeTab === 'gymInfo' && (
          <div className="p-6 bg-gray-50 rounded-lg shadow-inner">
            <h2 className="text-3xl font-semibold text-gray-700 mb-6">Update Gym Details</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
              <div>
                <label htmlFor="gymName" className="block text-gray-700 text-sm font-bold mb-2">
                  Gym Name
                </label>
                <input
                  type="text"
              ...
              <div className="flex flex-col items-center">
                <label htmlFor="gymLogo" className="block text-gray-700 text-sm font-bold mb-2">
                  Gym Logo
                </label>
                <img
                  src={gymLogoUrl}
                  ...
                  onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/150x150/CCCCCC/000000?text=No+Logo'; }}
                />
                <input
                  type="text"
                  id="gymLogo"
                ...
                />
                <p className="text-xs text-gray-500 mt-2">Enter a URL for the gym logo. (e.g., https://example.com/logo.png)</p>
              </div>
            </div>
        ...
        {activeTab === 'security' && (
          <div className="p-6 bg-gray-50 rounded-lg shadow-inner">
            <h2 className="text-3xl font-semibold text-gray-700 mb-6">Change Password</h2>
            <div className="mb-4">
            ...
            {passwordMessage && (
              <p className={`text-sm mb-4 ${passwordMessage.includes('successfully') ? 'text-green-600' : 'text-red-600'}`}>
                {passwordMessage}
              </p>
            )}
        ...
        {activeTab === 'membershipPlans' && (
          <div className="p-6 bg-gray-50 rounded-lg shadow-inner">
            <h2 className="text-3xl font-semibold text-gray-700 mb-6">Manage Membership Plans</h2>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <label htmlFor="planName" className="block text-gray-700 text-sm font-bold mb-2">
                    Plan Name
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label htmlFor="planPrice" className="block text-gray-700 text-sm font-bold mb-2">
                    Price ($)
                  </label>
                  <input
                    type="number"
                ...
                <div className="md:col-span-2">
                  <label htmlFor="planDuration" className="block text-gray-700 text-sm font-bold mb-2">
                    Duration
                  </label>
                  <input
                    type="text"
            ...
            {/* Existing Plans List */}
            <h3 className="text-2xl font-semibold text-gray-700 mb-4">Current Membership Plans</h3>
            <div className="space-y-4">
              {membershipPlans.length === 0 ? (
                <p className="text-gray-600 text-center py-4">No membership plans added yet.</p>
              ) : (
                membershipPlans.map((plan) => (
                  <div key={plan.id} className="bg-white p-6 rounded-lg shadow-md flex flex-col md:flex-row justify-between items-start md:items-center">
                    <div className="mb-4 md:mb-0">
                      <h4 className="text-xl font-bold text-gray-800">{plan.name}</h4>
                      <p className="text-gray-600">
                        <span className="font-semibold">${plan.price}</span> / {plan.duration}
                      </p>
